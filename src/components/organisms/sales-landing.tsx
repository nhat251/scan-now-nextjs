...
import { BarChart3, ChevronDown, HandCoins, Rocket, ScanLine, Sparkles, Users } from "lucide-react";
...
import {
  SalesLandingFeatureCta,
  SalesLandingFinalCtaActions,
  SalesLandingHeroActions,
  SalesLandingPricingCards,
} from "@/components/organisms/sales-landing-actions";
...
const HeroSection = async () => {
  ...
  return (
    <section
      id="hero"
      className="scroll-mt-header relative overflow-hidden bg-black px-5 pt-28 pb-24 md:px-16 md:pt-36"
    >
      <div
        aria-hidden
      ...
      <div
        aria-hidden
        className="from-background/95 via-background/70 to-background/15 pointer-events-none absolute inset-0 bg-gradient-to-r"
      />
      <div
        aria-hidden
        className="from-background/70 via-background/15 pointer-events-none absolute inset-0 bg-gradient-to-t to-transparent"
      />
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl">
        <div className="max-w-3xl space-y-8">
          ...
          <h1 className="text-on-surface text-4xl leading-tight font-extrabold sm:text-5xl lg:text-6xl">
            ...
            <span className="text-primary-container">{t("landing.hero.title.highlight")}</span>{" "}
          ...
          <div className="border-outline-variant/40 grid grid-cols-2 gap-6 border-t pt-8 sm:grid-cols-4 sm:gap-10">
            ...
            <MetricItem
              value={t("landing.hero.metrics.setupValue")}
              label={t("landing.hero.metrics.setup")}
            />
...
const FeatureDetailSection = async () => {
  ...
  return (
    <section
      id="feature-details"
      className="scroll-mt-header bg-surface-container-lowest px-5 py-24 md:px-16"
    >
      <div className="mx-auto w-full max-w-7xl space-y-16">
        ...
        <div className="from-primary-container to-secondary-container text-on-primary rounded-4xl bg-linear-to-r p-8 sm:p-12">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-3">
              <h3 className="text-2xl leading-tight font-bold sm:text-3xl">
                {t("landing.featureDetails.paymentCta.title")}
              </h3>
        ...
        <div className="border-outline-variant/40 bg-surface-container-low rounded-3xl border p-8 sm:p-10">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
            {processSteps.map((step, index) => (
              <article
                key={step.key}
                className="bg-surface-container-lowest border-outline-variant/35 rounded-2xl border p-6"
              >
  ...
  return (
    <section id="pricing" className="scroll-mt-header bg-surface-container-low px-5 py-24 md:px-16">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <SalesLandingPricingCards plans={planCards} />
      </div>
    </section>
  );
};
...
const FaqSection = async () => {
  ...
  const faqItems = [
    ...
    {
      key: "payments",
      question: t("landing.faq.items.payments.question"),
      answer: t("landing.faq.items.payments.answer"),
      defaultOpen: false,
    },
    {
      key: "menuLimit",
      question: t("landing.faq.items.menuLimit.question"),
      answer: t("landing.faq.items.menuLimit.answer"),
      defaultOpen: false,
    },
    {
      key: "qrDesign",
      question: t("landing.faq.items.qrDesign.question"),
      answer: t("landing.faq.items.qrDesign.answer"),
      defaultOpen: false,
    },
  ];
...
const FinalCtaSection = async () => {
  ...
  return (
    <section
      id="final-cta"
      className="scroll-mt-header bg-on-surface relative overflow-hidden px-5 py-24 md:px-16"
    >
      <div
        aria-hidden
        className="bg-primary/10 pointer-events-none absolute top-0 -right-24 h-full w-80 -skew-x-12"
      />
      <div
        aria-hidden
        className="bg-secondary/10 pointer-events-none absolute bottom-0 -left-20 h-full w-72 skew-x-12"
      />
